...
LangGraph是一个编排框架，可以用来构建大模型Agent，之前写过一个具体的使用例子: [使用LangGraph及MCP实现Agent](/2025/04/26/langchain-mcp/)，因为它本身已经内置了很多的组件，所以可以看到开发起来是比较简单的

本次主要是对其中的部分常用概念进行简单的介绍以更好的使用，更详细的内容可以参考[官方文档](https://langchain-ai.github.io/langgraph/)
...
State是一个数据结构，简单可以理解为一次对话过程中的数据信息，比如用户输入的内容以及大模型处理的响应信息等等，一般会继承`TypedDict`或者 Pydantic 的 `BaseModel`

通常我们会使用State 来存储我们与大模型整个交互过程中的消息内容，这时可以定义如下State
...
现在我们再来定义一下节点和流程，可以先不关注其中的具体细节，只要看state 相关部分
...
同时需要提供一个reduce函数，来将新增的消息进行合并，而不是进行覆盖，具体可以看下如下实现
...
Node是一个python 函数的节点，用于接收 state 信息，进行逻辑处理后，返回要更新的数据
...
node函数还可以增加一个可选的 RunnableConfig 类型参数，用于接收运行时的配置信息

```python
from langchain_core.runnables import RunnableConfig

def say_hello_node(state: ChatState, config: RunnableConfig):
    # 通过 RunnableConfig 可以获取允许是设置的配置信息
...
对于Node 来说，有两个特殊的节点需要注意一下，它们分别是起始的`START`节点和结束的`END`节点

构建LangGraph 时必须以`START`开始，以`END`结束
...
**普通边**: `graph.add_edge("node_1", "node_2")` ，表示执行完node_1后一定会执行node_2

**条件边**: `graph.add_conditional_edges(node_1, routing_func)`，表示执行完node1之后，会根据routing_func返回的节点名称进行路由，或者如果函数返回的内容和节点名称不匹配，可以在最后增加一个dict参数，将routing_func输出的结果映射到节点名称，如`graph.add_conditional_edges("node_1", routing_func, {"key1", "node2", "key2": "node3"})`

**入口点**: 定义开始后执行的第一个节点，`graph.add_edge(START, "node_1")`或者`graph.set_entry_point("node_1")`

**条件入口点**：可以在开始执行时就进行条件路由，`graph.add_conditional_edges(START, routing_func)`
...
默认情况下，当一个对话完成，state 中记录的全部信息就会释放，后续再次对话需要重新开始，大模型并没有之间的记忆

在多轮对话的过程中，我们希望Agent 能记住上几轮的对话内容，这时我们需要的就是LangGraph提供的短期记忆(short-term memory)；如果我们希望大模型能记住用户是一个什么样的人，后续可以提供更加个性化的回答，那就需要用到长期记忆(long-term memory)，下面我们来分别看下

#### 短期记忆

checkpointer 会将每个节点对应的state 信息进行记录
